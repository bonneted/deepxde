...
class ResidualBasedAttention(Callback):
    def __init__(self, residual_op, gamma=0.999, eta=0.001):
        super().__init__()
        self.gamma = gamma
        self.eta = eta
    ...
    def init(self):
        if backend_name != "jax":
            raise ValueError("ResidualBasedAttention only supports JAX backend currently.")
        self.train_x = self.model.data.train_x
        if isinstance(self.train_x, list):
            train_x_grid = transform_coords(self.train_x)
        self.rsum = jax.numpy.zeros((train_x_grid.shape[0], 1))
        self.model.train_state.train_aux_vars = [self.rsum]
        self.model.train_state.test_aux_vars = [self.rsum]

        # @jax.jit
        def residual_fn(params,x):
            def outputs_fn(inputs):
                return self.model.net.apply(params, inputs, True)
            y = outputs_fn(x)
            return self.residual_op(x, (y, outputs_fn))
        self.residual_fn = residual_fn

    # def on_train_begin(self):
    #     # Initialize rsum at the beginning of training
    #     if isinstance(self.train_x, list):
    #         train_x_grid = transform_coords(self.train_x)
    #     self.rsum = jax.numpy.zeros((train_x_grid.shape[0], 1))
    #     # Save to train_aux_vars so DeepXDE can pass it to the loss function
    #     self.model.train_state.train_aux_vars = [self.rsum]
    #     self.model.train_state.test_aux_vars = [self.rsum]

    def on_epoch_begin(self):
        # Compute updated residual weights rsum each epoch
        # train_x = self.model.data.train_x[0]
        # if isinstance(train_x, list):
        #     train_x = transform_coords(train_x)

        # Predict current PDE residuals
        residual = self.residual_fn(self.model.net.params, self.train_x[0])
        # Compute normalized residual magnitude
        r_norm = self.eta * jax.numpy.abs(residual) / jax.numpy.max(jax.numpy.abs(residual))
        # Exponential moving average
        self.rsum = self.gamma * self.rsum + r_norm
        # Update aux vars for next training step
        self.model.train_state.train_aux_vars = [self.rsum]
        self.model.train_state.test_aux_vars = [self.rsum]
...
def transform_coords(x):
    x_mesh = [
        x_.ravel()
        for x_ in jax.numpy.meshgrid(
            jax.numpy.atleast_1d(x[0].squeeze()),
            jax.numpy.atleast_1d(x[1].squeeze()),
            indexing="ij",
        )
    ]
    return jax.numpy.stack(x_mesh, axis=-1)